...
def connect_to_linkedin(driver, alert):
    job_object = job_search(driver, alert)
...
def harvest_opportunities():
    ...
    for alert in alerts:
        try:
            talk_to_slack(f"Searching for {alert} jobs...")
            jobs, exception = connect_to_linkedin(driver, alert)
            if exception != None:
                error_to_slack(f"I had some trouble getting all of the pages of jobs, but I was able to get some. Here is the error I enountered trying to go through the pages: {str(exception)}")
